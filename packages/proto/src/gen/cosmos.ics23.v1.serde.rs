...
impl serde::Serialize for InnerOp {
        ...
        S: serde::Serializer,
    {
        ...
        if self.hash != 0 {
            let v = HashOp::from_i32(self.hash).ok_or_else(|| {
                serde::ser::Error::custom(format!("Invalid variant {}", self.hash))
            })?;
